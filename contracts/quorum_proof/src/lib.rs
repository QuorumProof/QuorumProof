...
impl QuorumProofContract {
    ...
    pub fn issue_credential(
        ...
        env.storage().instance().extend_ttl(STANDARD_TTL, EXTENDED_TTL);
        let mut subject_creds: Vec<u64> = env.storage().instance()
    ...
    /// Retrieve a credential by ID. Panics if expired or not found.
    ...
    /// Returns the number of attestations for a credential.
    ...
    pub fn get_credential_type(env: Env, type_id: u32) -> CredentialTypeDef {
        env.storage().instance().get(&DataKey::CredentialType(type_id))
...
mod tests {
    use super::*;
    use soroban_sdk::{testutils::{Address as _, Ledger as _, LedgerInfo}, Bytes, Env};
    ...
    fn test_unauthorized_revoke_credential() {
        ...
        let metadata = Bytes::from_slice(&env, b"ipfs://QmTest");
        let id = client.issue_credential(&issuer, &subject, &1u32, &metadata, &None);
        client.revoke_credential(&Address::generate(&env), &id);
    }
    ...
    fn test_add_attestor_unauthorized_panics() {
        ...
        initial.push_back(attestor.clone());
        let slice_id = client.create_slice(&creator, &initial, &1u32);
        client.add_attestor(&Address::generate(&env), &slice_id, &Address::generate(&env));
    }
    ...
    fn test_update_threshold_success() {
        ...
        assert_eq!(client.get_slice(&slice_id).threshold, 1);
    }
    ...
    fn test_update_threshold_unauthorized_panics() {
        ...
        attestors.push_back(Address::generate(&env));
        let slice_id = client.create_slice(&creator, &attestors, &1u32);
        client.update_threshold(&Address::generate(&env), &slice_id, &1u32);
    }
    ...
    fn test_get_credential_count() {
        ...
        client.issue_credential(&issuer, &subject, &3u32, &metadata, &None);
    ...
    fn test_register_and_get_credential_type() {
        ...
        let desc = soroban_sdk::String::from_str(&env, "Bachelor or higher");
